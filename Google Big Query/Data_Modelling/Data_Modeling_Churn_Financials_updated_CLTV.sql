--
-- DATA MODELING
-- CHURNERD FINANCIAL METRICS (30 day subscription_date)
WITH user_data AS (
  SELECT
...
churners_financials AS (
  SELECT
    ...
    DATETIME_DIFF(DATE(t2.subscriptions_cancelled_at),t1.subscriber_since_timestamp,DAY) AS days_to_churn,
    t1.mrr,
    t1.arr,
    t1.net_payments
  FROM 
    db_table_financials AS t1
  JOIN db_table_subscriptions_cancellations_data_ab AS t2
...
financial_metrics AS (
  SELECT
    ...
    --CLTV
    ROUND(safe_divide(SUM(net_payments), COUNT(UUID)),2) AS clv_per_customer, -- raw cltv per customer
    ROUND(SAFE_DIVIDE(
      SAFE_DIVIDE(SUM(net_payments), COUNT(UUID)),
      SAFE_DIVIDE(AVG(churners_financials.days_to_churn), 30)
    ),2) AS monthly_clv, -- ARPU/(AVG_days_to_churn/30)
    ROUND(SAFE_DIVIDE(
      SAFE_DIVIDE(SUM(net_payments), COUNT(UUID)),
      SAFE_DIVIDE(AVG(churners_financials.days_to_churn), 365)
    ),2) AS annual_clv -- ARPU/(AVG_days_to_churn/365)
...
SELECT 
  ...
  -- fm.subscription_date_total_customer_projected_MRR, -- not inclusive in dataset
  -- fm.subscription_date_total_customer_projected_ARR, -- not inclusive in dataset
